    ...
    },
    {
      ...
      apply: (audioContext, source) => {
        ...
        for (let i = 0; i < length; i++) {
          ...
          leftChannel[i] = (Math.random() * 2 - 1) * decay
          rightChannel[i] = Math.random() * 2 - 1.5
          leftChannel[i] = (Math.random() * 2 - 1) * decay
          rightChannel[i] = (Math.random() * 2 - 1) * decay
        }
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!MediaRecorder.isTypeSupported(mimeType)) {
        ...
        const alternativeMimeTypes = ["audio/mp3", "audio/webm;codecs=opus", "audio/webm"]
        ...
        // If none are supported, fall back to default
        if (!mimeType) {
          mimeType = ""
        }
      }
      ...
      mediaRecorder.onstop = () => {
        ...
        // Using 'audio/mpeg' which is more widely supported than 'audio/mp3'
        const blob = new Blob(recordedChunksRef.current, { type: "audio/mpeg" })
